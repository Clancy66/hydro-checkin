...
打卡签到会随机掉落 $1\sim 3$ 枚金币，连续签到 7 天及以上则每天额外掉落 3 金币，该数据会写入数据库与 `hydro-coin` 插件联动，安装 `hydro-coin` 之后可以查看金币账单与商城。
...
- width: 4          # 配置在右侧边栏，默认宽度为 3，建议调整为 4 会比较美观
  checkin:
    coins: true     # 是否掉落金币，这里其实是会掉落的，只是配置为 false 则前端不展示，懒得改了，因为懒
    type:           # 配置主运势描述和配色
      - text: "AK IOI"
        color: "#ED5A65"
      - text: "AK APIO"
...
|`checkinReward`|`number`|最新一次签到随机的金币数，包含额外奖励|
